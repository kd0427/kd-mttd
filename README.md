...
| 💧 가로 요약바 | 게임 위에 작고 긴 요약바를 띄워 중요한 숫자만 빠르게 봅니다. |
| ⏱️ 맵 시간만 측정 | 기본값은 **맵 안에 있을 때만** 시간이 흐릅니다. 마을·메뉴에서는 멈춥니다. |
| ⏯️ 바로 제어 | 요약바 또는 앱에서 일시정지·재개·초기화를 할 수 있습니다. |
...
| 오버레이 권한 | 선택 | 게임 화면 위에 요약바와 HUD를 띄울 때 필요합니다. |
...
5. **오버레이 권한 허용** — `다른 앱 위에 표시`를 허용하면 게임 위 HUD를 사용할 수 있습니다.
...
이 작업은 현재 가방 상태를 기준점으로 만드는 과정입니다. 완료 전 요약바에는 주황색 신호와 `로그·가방 정리`가 표시됩니다. 기준점이 준비되면 아이템 획득과 소비가 집계됩니다. 🎒

## 💧 게임 위 요약바 사용법

`설정 → 오버레이 → 오버레이 표시`를 누르면 게임 화면 위에 가로 요약바가 뜹니다.

- **요약바를 탭** → 자세한 HUD를 엽니다.
- **자세한 HUD를 탭** → 다시 요약바로 접습니다.
- **길게 누른 뒤 드래그** → 원하는 위치로 옮깁니다.
- **⏯ 버튼** → 일시정지 또는 재개합니다.
- **🔄 버튼을 1초 길게 누르기** → 세션을 초기화합니다.
...
| 🟠 주황 | 로그·가방 정리 | 로그 오픈 또는 가방 정렬이 아직 필요합니다. |
...
> 기본 시간 기준은 **맵 안에서만 누적**입니다. 앱의 `설정 → 오버레이 → 시간 집계 기준`에서 가방 정렬 이후 항상 누적하도록 바꿀 수 있습니다.
...
| **가치** | 현재 보유 아이템의 예상 가치를 확인 |
| **설정** | Shizuku 상태, 오버레이, 시간 기준, 시세 출처, 로그 진단 설정 |
