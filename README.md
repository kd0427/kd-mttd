...
![Android 11+](https://img.shields.io/badge/Android-11%20%28API%2030%29%2B-3DDC84?logo=android&logoColor=white)
...
설치한 뒤에는 [처음 사용하는 방법](#-처음-사용하는-방법)을 따라 무선 디버깅 연결만 한 번 해주면 됩니다.

> 📦 **지난 버전도 [릴리스 페이지](https://github.com/kd0427/kd-mttd/releases)에 그대로 남아 있습니다.**
>
> **Android 10 기기라면 [v0.5.16](https://github.com/kd0427/kd-mttd/releases/tag/v0.5.16)을 받으세요.** v0.6.0부터는 Android 11 이상만 설치됩니다 — 게임 로그를 읽는 방식이 무선 디버깅 기반으로 바뀌었고, 그 기능이 Android 11부터라서입니다. v0.5.16은 예전처럼 [Shizuku](https://shizuku.rikka.app/)를 씁니다.
>
> ⚠️ 최신 버전에서 예전 버전으로 되돌리려면 앱을 지웠다 깔아야 하고, 그때 집계 기록이 사라집니다 (안드로이드가 낮은 버전 덮어쓰기를 막습니다).
...
| 항목 | 필요 여부 | 내용 |
|---|---|---|
| Android 11 이상 | 필수 | API 30 이상 (무선 디버깅이 Android 11부터 있는 기능입니다) |
| Torchlight: Infinite | 필수 | 글로벌/탭/중국 패키지를 자동으로 찾습니다. |
| 무선 디버깅 | 필수 | 개발자 옵션에서 켭니다. 다른 앱 설치나 root는 필요 없습니다. |
| USB 디버깅 | 필수 | 같은 개발자 옵션에서 켭니다. **케이블은 꽂지 않아도 됩니다** — 꺼져 있으면 WiFi를 끄는 순간 연결이 끊깁니다. |
| 게임 위 표시 권한 | 선택 | 게임 화면 위에 미니패널과 상세 패널을 띄울 때 필요합니다. |

Android 11부터 일반 앱은 다른 앱의 `Android/data` 파일에 바로 접근할 수 없습니다. 고인물 mTTD는 기기의 무선 디버깅에 **자기 자신으로** 붙어서 게임 로그를 **읽기만** 합니다 — 예전처럼 Shizuku 같은 별도 앱을 설치할 필요가 없습니다.
...
1. **개발자 옵션 켜기** — `설정 → 휴대전화 정보 → 빌드번호`를 7번 연달아 탭하면 나타납니다.
2. **USB 디버깅과 무선 디버깅을 둘 다 켜기** — `개발자 옵션` 안에 둘 다 있습니다. 케이블은 꽂지 않아도 됩니다.
3. **페어링 시작** — 고인물 mTTD에서 `페어링 시작`을 누릅니다. 앱을 나가도 계속 찾습니다.
4. **페어링 코드 열기** — `개발자 옵션 → 무선 디버깅 → 페어링 코드로 기기 페어링`을 엽니다. ⚠️ 이 화면에서 나가면 처음부터 다시 해야 합니다.
5. **코드 입력** — 그 화면을 둔 채 알림창을 내려, 뜬 알림에 6자리 코드를 입력합니다. 시간 제한은 없으니 천천히 하셔도 됩니다.
6. **게임 위 표시 권한 허용** — `다른 앱 위에 표시`를 허용하면 게임 위 패널을 사용할 수 있습니다.

> 📶 연결이 끝나면 **WiFi를 꺼도(LTE 등)** 계속 동작합니다. 설정 탭의 카드가 "이제 WiFi 를 꺼도 됩니다"로 바뀐 뒤에 끄세요.
>
> ⚠️ **USB 디버깅을 끄면 안 됩니다.** 끈 상태로 WiFi까지 끄면 연결이 끊깁니다 — 자세한 이유는 [CUSTOMIZATION.md](CUSTOMIZATION.md)에 있습니다.
>
> ⚠️ **무선 디버깅은 가끔 저절로 꺼집니다.** 재부팅했을 때, WiFi가 잠깐 끊겼을 때(화면 꺼짐 절전, 공유기 재접속, 다른 WiFi로 이동 등), 오래 안 썼을 때 Android가 알아서 끕니다 — 앱 문제가 아니라 Android 자체 동작이라 막을 수 없습니다. 연결이 끊겼다고 나오면 `개발자 옵션 → 무선 디버깅`을 다시 켜고, WiFi에 연결한 상태로 앱을 한 번 열어주세요. **페어링은 저장돼 있어서 6자리 코드는 다시 입력하지 않아도 됩니다.**
...
- **미니패널을 탭** → 상세 패널을 엽니다.
- **상세 패널에서 좌우로 쓸기**(또는 `수익`/`가치` 글자 탭) → 수익 화면과 보유 가치 화면을 오갑니다. 거래소에 들어가면 자동으로 가치 화면이 뜹니다.
- **상세 패널을 탭** → 다시 미니패널로 접습니다.
- **길게 누른 뒤 드래그** → 원하는 위치로 옮깁니다.
- **⏯ 버튼** → 일시정지 또는 재개합니다.
- **🗑 버튼** → 세션을 초기화합니다. 미니패널에서는 두 번, 상세 패널에서는 한 번 누르면 실행됩니다.
- **🔄 버튼**(상세 패널 · 가치 화면) → 보유 아이템 가치를 다시 계산합니다. 세션은 그대로입니다.
...
| **설정** | 무선 adb 연결 상태, 게임 화면 표시, 시간 측정 방식, 시세 출처, 로그 연결 상태 |
...
| 항목 | 처리 방식 |
|---|---|
| 게임 로그 | 무선 adb 연결로 읽기만 합니다. 파일을 만들거나 고치거나 삭제하지 않습니다. |
| 수익 기록 | 기기 안에만 저장됩니다. |
| 네트워크 | 시세, 앱 업데이트 확인, 아이템 아이콘에 사용합니다. 캐릭터 빌드 내보내기는 버튼을 직접 누를 때만 전송합니다. |
...
- `mini-tlidb.winterer.workers.dev` — 캐릭터 빌드 내보내기. `설정` 에서 버튼을 **직접 누를 때만** 전송하며, 로그인 시 게임이 내려주는 캐릭터 스냅샷 로그 원문을 올리고 1회용 열람 URL을 받습니다. 올린 데이터는 10분간 열어보지 않으면 자동으로 폐기됩니다. 자동으로는 전송되지 않습니다.
