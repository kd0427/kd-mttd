...
업데이트 확인은 `/releases/latest` 만 보므로 옛 릴리스는 업데이트 경로로는 아무 역할이 없다.

**최신 3개까지만 남긴다.** 새 버전을 낸 뒤 네 번째부터를 지운다 — 되돌릴 일이 생겼을 때
직전 두 버전의 APK 는 손에 있어야 한다.

> ### ⚠️ `v0.5.16` 은 이 규칙에서 제외한다 — 지우지 말 것
>
> v0.6.0 에서 `minSdk` 가 29 → 30 으로 올라가(무선 디버깅이 Android 11+ 기능) **Android 10
> 기기는 0.6.x 를 설치할 수 없다.** `v0.5.16` 은 그 기기들이 쓸 수 있는 마지막 버전이고
> (Shizuku 방식), 지우면 그 사용자들에게 남는 경로가 없다. README 도 Android 10 사용자를
> 이 릴리스로 안내한다.
>
> Android 10 을 더 이상 신경 쓰지 않기로 결정하면 그때 이 문단과 README 안내를 같이 지운다.
