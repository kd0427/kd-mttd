...
data class SessionState(
    ...
    /**
     * 로그는 읽히는데 더 이상 안 자란다 = 게임이 안 돌고 있다 (종료·강제종료·백그라운드 정리).
     *
     * 표시 전용이다 — 시계는 [paused] 가 멈춘다 (게임이 없는 동안을 판 시간으로 세면 안 되므로
     * 집계기가 자동으로 pause 를 건다). [logStalled] 와는 구분해야 한다: 저건 "우리가 못 읽는다",
     * 이건 "읽히는데 게임이 없다" 라서, 전자는 판단 자체가 불가능하고 후자는 확정이다.
     */
    val gameAway: Boolean = false,
) {
    /**
