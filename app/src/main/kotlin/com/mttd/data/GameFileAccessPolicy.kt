 ...
 *
 * 두 구현이 각자 다른 경로로 파일에 접근한다 — [UserService][com.mttd.service.UserService]
 * (shell UID 데몬 프로세스)와 `DirectAdbManager.LocalUserService`(데몬이 뜨기 전 폴백, adb 셸
 * 명령을 직접 실행). **어느 쪽이든 이 whitelist를 통과 못 한 경로는 절대 건드리면 안 된다** —
 * 한쪽만 고치고 다른 쪽을 깜빡하면 보안 경계가 갈라지므로 반드시 이 한 곳만 수정한다.
 */
