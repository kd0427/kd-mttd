...
> Note: this fork ships a single build with no product flavors. Upstream (`listil/mttd`) keeps this
> code in a separate `direct` flavor; here it lives in `app/src/main/**`. See `CUSTOMIZATION.md`.

## RikkaApps/Shizuku (Apache License 2.0)

The wireless-debugging pairing/connection code is ported from
...
- `app/src/main/kotlin/com/mttd/data/adb/AdbKey.kt`
- `app/src/main/kotlin/com/mttd/data/adb/AdbClient.kt`
- `app/src/main/kotlin/com/mttd/data/adb/AdbPairingClient.kt`
- `app/src/main/kotlin/com/mttd/data/adb/AdbMessage.kt`
- `app/src/main/kotlin/com/mttd/data/adb/AdbProtocol.kt`
- `app/src/main/kotlin/com/mttd/data/adb/AdbException.kt`
- `app/src/main/jni/adb_pairing.cpp`
- `app/src/main/jni/adb_pairing.h`
- `app/src/main/jni/logging.h`
- `app/src/main/jni/mttd_starter.cpp` (ported from `manager/src/main/jni/starter.cpp` and
  `misc.cpp` — root-only code paths removed, since mTTD only ever runs as the adb shell user;
  see the file's own header comment for the exact adaptation)
...
**2026-08-15 update**: mTTD *does* now spawn a second process — a shell-UID
...
(`app/src/main/kotlin/com/mttd/data/adb/starter/HiddenApis.kt`,
...
`org.conscrypt:conscrypt-android` is used to call the public
