...
object HiddenApis {
    ...
    fun callProvider(authority: String, callingPkg: String, method: String, extras: Bundle) {
        // token 을 넘기고 끝나면 반드시 돌려준다. null 을 넘기면 AMS 가 "회수 불가" 카운트
        // (externalProcessNoHandleCount)를 올리는데, 그건 removeContentProviderExternal 로도
        // 못 내려서 provider 레코드가 영영 정리 대상에서 빠진다 — 호출할 때마다 누적된다.
        val token = Binder()
        val provider = getContentProviderExternal(authority, 0, token, authority)
        ...
        try {
            ...
            releaseContentProviderExternal(authority, token)
        }
    }

    /**
     * [getContentProviderExternal] 로 잡은 외부 참조를 돌려준다.
     *
     * 실패해도 무시한다 — 이 시그니처가 기기/버전에 따라 다를 수 있는데, 못 돌려줘도 우리 토큰에
     * 걸린 linkToDeath 덕분에 데몬이 죽을 때 AMS 가 정리한다(= 최악이라도 데몬 수명으로 한정된다).
     */
    private fun releaseContentProviderExternal(name: String, token: IBinder) {
        try {
            val am = getActivityManager()
            am.javaClass
                .getMethod("removeContentProviderExternal", String::class.java, IBinder::class.java)
