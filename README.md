...
1. **무선 디버깅 켜기** — 휴대폰의 `개발자 옵션 → 무선 디버깅`을 켭니다. 개발자 옵션이 안 보이면 `설정 → 휴대전화 정보 → 빌드번호`를 7번 연달아 탭하면 나타납니다.
2. **페어링 시작** — 고인물 mTTD에서 `페어링 시작`을 누르고 **그대로 둡니다**.
3. **페어링 코드 열기** — `개발자 옵션 → 무선 디버깅 → 페어링 코드로 기기 페어링`을 엽니다. ⚠️ 이 화면을 벗어나면 페어링이 끊기니 화면을 바꾸지 마세요.
4. **코드 입력** — 알림 서랍을 내려서 뜬 알림에 화면의 6자리 코드를 입력하면 자동으로 연결됩니다.
5. **게임 위 표시 권한 허용** — `다른 앱 위에 표시`를 허용하면 게임 위 패널을 사용할 수 있습니다.

> 📶 한 번 연결되면 그 뒤로는 **WiFi를 꺼도(LTE 등)** 계속 동작합니다.
>
